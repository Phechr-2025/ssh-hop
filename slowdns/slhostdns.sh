...
NS_DOMAIN=ns-$DOMAIN
CF_ZONE="${PHECHR_CF_ZONE:-${CF_ZONE:-$DOMAIN}}"
CF_ID="${PHECHR_CF_EMAIL:-${CF_EMAIL:-}}"
CF_KEY="${PHECHR_CF_KEY:-${CF_KEY:-}}"
[ -z "$CF_ID" ] && read -rp "Cloudflare email: " CF_ID
if [ -z "$CF_KEY" ]; then
...
ZONE=$(curl -sLX GET "https://api.cloudflare.com/client/v4/zones?name=${CF_ZONE}&status=active" \
